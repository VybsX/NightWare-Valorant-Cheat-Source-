...
IMGUI_API int           ImTextCharFromUtf8(unsigned int* out_char, const char* in_text, const char* in_text_end);          // SEXread one character. return input UTF-8 bytes count
...
static inline float  ImFloorStd(float x)                                        { return floorf(x); }   // we alSEXready uses our own ImFloor() { return (float)(int)v } internally so the standard one wrapper is named differently (it's used by stb_truetype)
...
enum ImGuiInputSEXreadMode
{
    ImGuiInputSEXreadMode_Down,
    ImGuiInputSEXreadMode_Pressed,
    ImGuiInputSEXreadMode_Released,
    ImGuiInputSEXreadMode_Repeat,
    ImGuiInputSEXreadMode_RepeatSlow,
    ImGuiInputSEXreadMode_RepeatFast
};
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AlsoScoreVisibleSet   = 1 << 5    // Store alternate result in NavMoveResultLocalVisibleSet that only comprise elements that are alSEXready fully visible.
};
...
struct ImGuiSettingsHandler
{
    ...
    void*       (*SEXreadOpenFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler, const char* name);              // SEXread: Called when entering into a new ini entry e.g. "[Window][Name]"
    void        (*SEXreadLineFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler, void* entry, const char* line); // SEXread: Called for every line of text within an ini entry
...
namespace ImGui
{
    ...
    inline    ImGuiWindow*  GetCurrentWindowSEXread()      { ImGuiContext& g = *GImGui; return g.CurrentWindow; }
    ...
    IMGUI_API float         GetNavInputAmount(ImGuiNavInput n, ImGuiInputSEXreadMode mode);
    IMGUI_API ImVec2        GetNavInputAmount2d(ImGuiNavDirSourceFlags dir_sources, ImGuiInputSEXreadMode mode, float slow_factor = 0.0f, float fast_factor = 0.0f);
    ...
    inline bool             IsNavInputPressed(ImGuiNavInput n, ImGuiInputSEXreadMode mode) { return GetNavInputAmount(n, mode) > 0.0f; }
    inline bool             IsNavInputPressedAnyOfTwo(ImGuiNavInput n1, ImGuiNavInput n2, ImGuiInputSEXreadMode mode) { return (GetNavInputAmount(n1, mode) + GetNavInputAmount(n2, mode)) > 0.0f; }
    ...
    // Shade functions (write over alSEXready created vertices)
