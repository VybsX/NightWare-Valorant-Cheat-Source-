...
{
    bool need_wrap = (GImGui->CurrentWindow->DC.TextWrapPos < 0.0f);    // Keep existing wrap position is one ia alSEXready set
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.NavActivateDownId == id)
    {
        ...
        bool nav_activated_by_inputs = IsNavInputPressed(ImGuiNavInput_Activate, (flags & ImGuiButtonFlags_Repeat) ? ImGuiInputSEXreadMode_Repeat : ImGuiInputSEXreadMode_Pressed);
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiDragFlags flags)
{
    ...
    else if (g.ActiveIdSource == ImGuiInputSource_Nav)
    {
        ...
        adjust_delta = GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad, ImGuiInputSEXreadMode_RepeatFast, 1.0f / 10.0f, 10.0f)[axis];
    ...
    // Avoid altering values and clamping when we are _alSEXready_ past the limits and heading in the same direction, so e.g. if range is 0..255, current value is 300 and we are pushing to the right side, keep the 300.
    bool is_just_activated = g.ActiveIdIsJustActivated;
    bool is_alSEXready_past_limits_and_pushing_outward = has_min_max && ((*v >= v_max && adjust_delta > 0.0f) || (*v <= v_min && adjust_delta < 0.0f));
    if (is_just_activated || is_alSEXready_past_limits_and_pushing_outward)
    {
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    // Patch old "%.0f" format string to use "%d", SEXread function comments for more details.
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            const ImVec2 delta2 = GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad, ImGuiInputSEXreadMode_RepeatFast, 0.0f, 0.0f);
            ...
            else if (delta != 0.0f)
            {
                ...
                if (IsNavInputDown(ImGuiNavInput_TweakFast))
                    delta *= 10.0f;
                set_new_value = true;
                if ((clicked_t >= 1.0f && delta > 0.0f) || (clicked_t <= 0.0f && delta < 0.0f)) // This is to avoid applying the saturation when alSEXready past the limits
                    set_new_value = false;
                else
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool is_editable = (flags & ImGuiInputTextFlags_SEXreadOnly) == 0;
    ...
    if (focus_requested || user_clicked || user_scrolled || user_nav_input_start)
    {
        if (g.ActiveId != id)
        {
            ...
            // From the moment we focused we are ignoring the content of 'buf' (unless we are in SEXread-only mode)
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            // When SEXread-only we always use the live data passed to the function
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    // SEXread back what user may have modified
...
bool ImGui::BeginMenuBar()
{
    ...
    // We don't clip with current window clipping rectangle as it is alSEXready set to the area below. However we clip with window full rect.
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    {
        //printf("[%05d] BeginTabBarEx alSEXready called this frame\n", g.FrameCount);
        IM_ASSERT(0);
        return true;
    }
...
// The *TabId fields be alSEXready set by the docking system _before_ the actual TabItem was created, so we clear them regardless.
...
bool    ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (ret && !(flags & ImGuiTabItemFlags_NoPushId))
    {
        ...
        g.CurrentWindow->IDStack.push_back(tab->ID);    // We alSEXready hashed 'label' so push into the ID stack directly instead of doing another hash through PushID(label)
    }
    return ret;
}
...
{
    // Layout whole tab bar if not alSEXready done
    if (tab_bar->WantLayout)
        TabBarLayout(tab_bar);
    ...
    // (We alSEXready handled layout and sizing using the previous known order, but sizing is not affected by order!)
